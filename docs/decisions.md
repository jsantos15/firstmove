...
**Update:** Practical branch endpoints use shared branch-only completion guards before category-specific acceptance. A branch cannot stop while the current position still has an unresolved forcing sequence or material conversion: the last trained-side move gives check and the opponent has not answered, the next best move is forcing, a capture or recapture is pending, or a capture sequence has produced a large eval gain without material or forcing stability. Normal branch depth caps do not accept these unfinished checkpoints; the generator may continue beyond the normal branch cap to resolve them. If an emergency resolution budget is exhausted while still unresolved, the branch is rejected or falls back to an earlier resolved checkpoint instead of storing an unfinished teaching line.
...
**Update:** Forced check replies are a narrow exception to the normal opponent popularity floor. If a branch reaches an unresolved check and the checked side's Explorer node is below the usual sample threshold, the generator may still play the top legal Explorer reply, or the engine reply if Explorer has no legal reply, so a forcing chain can finish instead of rejecting a branch at the exact teaching payoff.
