...
**Update:** After a branch has reached a strong trained-side advantage, the search still narrows to the top engine continuation by default, but it must also keep bounded material-recovery candidates. A checked trained move that directly attacks recoverable opponent material may stay in the candidate set when it is within the material-recovery eval-loss window, even if it is outside the normal trained-candidate loss threshold. This prevents practical branches from stopping at an unresolved high-eval fallback one move before the learner can force the opponent to answer check and then recover hanging material.

**Update:** `payoff_cap` fallback must not keep an unresolved position when it is the trained side's turn to move. If the current state still has unresolved forcing or material conversion and the learner has the move, the line should either continue to a real endpoint or be omitted from the selected branch set; it should not become a practice row that stops before the learner's next forcing or material-recovery move.
