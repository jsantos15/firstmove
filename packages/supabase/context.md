...
- **`src/database.types.ts`** — TypeScript types matching the Supabase schema (auto-generated by CLI)
...
- **Query functions:** verb + noun — `getUserProgress`, `upsertProgress`, `createRepertoire`
- **Migration files:** `NNN_description.sql` — `001_initial_schema.sql`, `002_add_streaks.sql`
...
2. Update `src/database.types.ts` by re-running the Supabase CLI type generator (see `scripts/setup-db.md`)
...
- `database.types.ts` is auto-generated — do not hand-edit it after the Supabase project is set up
